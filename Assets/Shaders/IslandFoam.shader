    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float node_9223 = (clamp(abs(tan(node_1897.g)),0.0,1.0)*(1.0 - o.uv0.g));
                v.vertex.xyz += (float3(node_9223,0.0,node_9223)*o.bitangentDir);
...
////// Emissive:
                float4 node_1897 = _Time + _TimeEditor;
                float2 node_8711 = (i.uv0+(node_1897.g/(_node_3981*9.0+1.0))*float2(1,0));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_8711, _MainTex));
                float3 emissive = (_MainTex_var.rgb*_TintColor.rgb*_TintColor.rgb*2.0);
